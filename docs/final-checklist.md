# Final Submission Checklist

## Ready links

- GitHub repository: https://github.com/gnanam1990/machinepay-grid
- Live demo: https://gnanam1990.github.io/machinepay-grid/
- Demo video: https://gnanam1990.github.io/machinepay-grid/docs/assets/video/machinepay-grid-demo.mp4
- Proposal: docs/proposal.md
- Form copy: docs/submission.md

## Pre-submit checks

- Open the live demo and click Force Buy four times.
- Confirm the proof packet shows Circle Nanopayments and an Arc batch id.
- Run `npm run arc:check`.
- Run `npm run server`, then confirm `curl -i http://localhost:3337/power` returns `402 Payment Required`.
- If a funded testnet EOA is available, run `npm run pay:real`.
- Open the demo video link and confirm the MP4 plays.
- Paste the project description from docs/submission.md.
- Use the GitHub repository and live demo links above.

## Questbook form fields

- Project name: MachinePay Grid
- Category: stablecoin payments, agent commerce, machine-to-machine payments
- Repository: https://github.com/gnanam1990/machinepay-grid
- Demo: https://gnanam1990.github.io/machinepay-grid/
- Video: https://gnanam1990.github.io/machinepay-grid/docs/assets/video/machinepay-grid-demo.mp4

## Remaining manual action

Log in to the Circle Questbook portal, review the pasted answers, connect any required wallet or account, and press Final submit only after the details look correct.
